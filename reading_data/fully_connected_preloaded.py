  ...
  with tf.Graph().as_default():
    with tf.name_scope('input'):
      ...
      image, label = tf.train.slice_input_producer(
          [input_images, input_labels], num_epochs=FLAGS.num_epochs, capacity=3000000)
      label = tf.cast(label, tf.int32)
      images, labels = tf.train.shuffle_batch(
          [image, label], batch_size=FLAGS.batch_size, capacity=3000000, num_threads=24, min_after_dequeue=300)
